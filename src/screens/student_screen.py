...
from src.database.db import get_all_students,create_student,get_students_subjects,get_students_attendances,unenroll_student_to_subject
import time 
...
def student_dashboard():
    ...
    with st.spinner('Loading your enrolled subjects...'):
        ...
        logs  = get_students_attendances(student_id)
    ...
    for i , sub_node in enumerate(subjects):
        ...
        def unenroll_button():
            if st.button("Unenroll form this cource", key=f"unenroll_{student_id}_{sid}", type='tertiary',width='stretch', icon=':material/delete_forever:'):
...
def student_screen():
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input('Enter your Name', placeholder="e.g Rohit Valvi")
                    ...
                    with st.spinner("Creating profile.."):
                        ...
                        if encodings:
                            ...
                            if response_data:
                                ...
                                created_name = response_data[0].get('name', new_name)
                                st.toast(f"Profile Created! Hi {created_name}")
                                time.sleep(1)
                                _safe_rerun()
                        else:
